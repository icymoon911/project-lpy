...
def __read_manager_plugins():
    ...
    try:
        ...
        # Always restore sys.path, even if an exception was raised above.
        sys.path = oldpathes
    return managers


# ---------------------------------------------------------------------------
# Thread-safe global manager cache
# ---------------------------------------------------------------------------
...
def get_managers():
    """Return the cached manager dictionary, creating it on first call.

    Protected by a lock so that concurrent callers from different threads
    cannot trigger a double-initialisation race.
    """
    global __MANAGERS
