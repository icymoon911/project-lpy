...
# ---------------------------------------------------------------------------
# Description-field → widget mapping
# ---------------------------------------------------------------------------
# Centralised here so that adding/removing a field only requires editing this
# constant, not hunting through __init__.  The keys are L-Py metadata field
# names; the values are the objectName of the corresponding QTextEdit in the
# main-window UI file.
# ---------------------------------------------------------------------------
DESC_ITEMS_CONFIG = {
    '__authors__':     'authorsEdit',
    '__institutes__':  'intitutesEdit',
    '__copyright__':   'copyrightEdit',
    '__description__': 'descriptionEdit',
    '__references__':  'referenceEdit',
}
...
class LPyWindow(QMainWindow, lsmw.Ui_MainWindow, ComputationTaskManager) :
    ...
    def __init__(self, parent=None, withinterpreter = True):
        ...
        self._init_preferences_defaults()
        self._init_docks_and_shell()
        self._init_desc_items()
        self._init_viewer_and_abort()
        self._init_editor_and_debugger()
        self._init_signals()
        self._init_menus_and_state()

        settings.restoreState(self)
        self._post_restore_setup()

    # ------------------------------------------------------------------
    # Initialisation sub-steps (extracted from the monolithic __init__)
    # ------------------------------------------------------------------

    def _init_preferences_defaults(self):
        """Set default values for preference-related attributes."""
    ...
    def _init_docks_and_shell(self):
        """Initialise dock widgets, shell, and recent-files icon."""
    ...
    def _init_desc_items(self):
        """Build the description-field → widget mapping from UI objectNames."""
        self.desc_items = {}
        for field_name, object_name in DESC_ITEMS_CONFIG.items():
            widget = self.findChild(QObject, object_name)
            if widget is not None:
                self.desc_items[field_name] = widget

    def _init_viewer_and_abort(self):
        """Set up the 3D viewer, plotter, and abort function."""
        self.viewer = Viewer
    ...
    def _init_editor_and_debugger(self):
        """Initialise the code editor, debugger, panel manager, and first file."""
        self.frameFind.hide()
    ...
    def _init_signals(self):
        """Connect all Qt signals to their slots."""
        # Document tabs
    ...
    def _init_menus_and_state(self):
        """Build menus, set initial widget state, and trigger version check."""
        self.initSVNMenu()
        self.printTitle()
    ...
    def _post_restore_setup(self):
        """Work that runs after settings have been restored."""
